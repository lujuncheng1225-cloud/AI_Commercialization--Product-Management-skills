const skills=[
{id:'competitor-feature',category:'竞品分析',title:'竞品功能对比',titleEn:'Feature Comparison',desc:'输入产品名，AI 帮你生成结构化功能对比矩阵',descEn:'Generate a structured feature comparison matrix for any two products',level:'入门',
steps:[
...
prompt:d=>`你是一个资深产品经理，擅长竞品分析。\n\n请对比以下两个产品：\n· ${d.productA}\n· ${d.productB}\n\n重点对比维度：\n${d.dimensions.map(v=>'· '+v).join('\n')}\n\n${d.context?'我的背景：'+d.context+'\n\n':''}请按以下结构输出：\n1. 市场定位对比\n2. 核心功能对比矩阵（用表格）\n3. 用户体验差异\n4. 差异化分析\n5. 结论与建议`,
tips:['把生成的 Prompt 粘贴到 ChatGPT 或 DeepSeek','可以追问"帮我补充用户评价方面的对比"','建议使用 GPT-4 或 Claude 获得最佳效果']},

{id:'competitor-pricing',category:'竞品分析',title:'竞品定价分析',titleEn:'Pricing Analysis',desc:'对比竞品定价策略，找到定价机会点',descEn:'Compare competitor pricing strategies and find pricing opportunities',level:'进阶',
steps:[
...
prompt:d=>`你是一个资深的 SaaS 产品经理，擅长定价策略分析。\n\n请分析以下产品的定价策略：\n${d.products.split('\n').filter(Boolean).map(v=>'· '+v).join('\n')}\n\n目标市场：${d.market}\n${d.pricing?'已知定价信息：\n'+d.pricing+'\n\n':''}重点分析维度：\n${d.focus.map(v=>'· '+v).join('\n')}\n\n请按以下结构输出：\n1. 各产品定价概览（表格）\n2. 定价模型对比分析\n3. 价格-功能对应关系\n4. 目标用户分层策略\n5. 定价机会与建议`,
tips:['不知道竞品定价也没关系，AI 会基于公开信息分析','追问"如果我要做低价切入，怎么定价合适"']},

{id:'competitor-tracking',category:'竞品分析',title:'竞品动态追踪',titleEn:'Competitive Tracking',desc:'生成竞品监控框架，系统化追踪竞品变化',descEn:'Build a competitive monitoring framework to track changes systematically',level:'入门',
steps:[
...
prompt:d=>`你是一个资深产品经理，擅长竞品情报收集与分析。\n\n请帮我建立一个竞品动态追踪框架：\n\n追踪目标：\n${d.competitors.split('\n').filter(Boolean).map(v=>'· '+v).join('\n')}\n\n重点追踪类型：\n${d.trackTypes.map(v=>'· '+v).join('\n')}\n\n请输出：\n1. 信息来源清单（每个竞品推荐 5-8 个信息渠道）\n2. 追踪频率建议\n3. 动态分类标签体系\n4. 一份可直接使用的竞品动态周报模板\n5. 关键信号识别指南`,
tips:['可以把生成的追踪框架放入 Notion / 飞书多维表格使用','追问"帮我设计竞品动态周报的具体格式"']},

{id:'user-persona',category:'用户研究',title:'用户画像生成',titleEn:'User Persona',desc:'输入产品信息，AI 生成详细的用户画像',descEn:'Generate detailed user personas based on your product info',level:'入门',
steps:[
...
prompt:d=>`你是一个资深用户研究员，擅长构建用户画像。\n\n产品：${d.product}\n目标用户：${d.target}\n使用场景：\n${d.scenarios.map(v=>'· '+v).join('\n')}\n\n请生成 3 个详细的用户画像，每个包含：\n1. 基本信息（虚构但合理）\n2. 用户目标与痛点\n3. 使用行为特征\n4. 决策因素\n5. 典型的一天（Day in the Life）\n6. 对我们产品的期待与担忧`,
tips:['生成的画像可用于 PRD、设计评审和营销文案','追问"帮我补充用户获取渠道分析"']},

{id:'user-feedback',category:'用户研究',title:'用户反馈分析',titleEn:'Feedback Analysis',desc:'粘贴用户反馈，AI 帮你提炼关键洞察',descEn:'Paste user feedback and extract key insights',level:'入门',
steps:[
...
prompt:d=>`你是一个资深产品经理，擅长用户反馈分析。\n\n以下是收集到的用户反馈：\n${d.feedback}\n\n请进行分析，重点：${d.focus}\n\n请输出：\n1. 反馈总览\n2. 高频关键词提取\n3. 功能需求分类与优先级建议\n4. 正面反馈亮点\n5. 负面反馈痛点及根因分析\n6. 行动建议（短期 vs 长期）`,
tips:['反馈越多分析越准确，建议至少 10 条','追问"帮我设计一个用户反馈收集问卷"']},

{id:'user-churn',category:'用户研究',title:'用户流失分析',titleEn:'Churn Analysis',desc:'分析用户为什么离开，找到挽回策略',descEn:'Analyze why users leave and find retention strategies',level:'进阶',
steps:[
...
prompt:d=>`你是一个资深增长产品经理，擅长用户留存和流失分析。\n\n产品：${d.product}\n\n流失数据：\n${d.data}\n\n怀疑的流失原因：\n${d.suspects.map(v=>'· '+v).join('\n')}\n\n请输出：\n1. 流失漏斗分析\n2. 流失用户画像特征\n3. 流失原因假设排序\n4. 验证假设的方案\n5. 挽回策略建议（按用户价值分层）\n6. 预防流失的长期策略`,
tips:['数据不多也没关系，AI 会基于常见模式给出分析框架','追问"帮我设计新用户 Onboarding 优化方案"']},

{id:'req-priority',category:'需求管理',title:'需求优先级排序',titleEn:'Priority Sorting',desc:'粘贴需求列表，AI 用专业框架帮你排序',descEn:'Sort requirements using professional frameworks (RICE, ICE, etc.)',level:'进阶',
steps:[
...
prompt:d=>`你是一个资深产品经理，擅长需求管理和优先级决策。\n\n以下是我的需求列表：\n${d.requirements}\n\n请使用 ${d.framework} 框架帮我排序。\n\n请输出：\n1. 每个需求的框架评分明细（用表格）\n2. 排序结果（从高到低）\n3. 分为三档：立即做 / 计划做 / 暂缓\n4. 如果只能选 3 个，推荐哪 3 个？为什么？`,
tips:['需求描述越详细，排序越准确','追问"帮我补充每个需求的工作量估算"']},

{id:'prd-draft',category:'需求管理',title:'PRD 初稿生成',titleEn:'PRD Draft',desc:'输入需求信息，AI 生成结构化 PRD 初稿',descEn:'Generate a structured PRD draft from your requirements',level:'进阶',
steps:[
...
prompt:d=>`你是一个资深产品经理，擅长撰写 PRD。\n\n需求：${d.requirement}\n目标用户：${d.users}\n\n核心功能：\n${d.features.split('\n').filter(Boolean).map(v=>'· '+v).join('\n')}\n\n约束条件：\n${d.constraints.map(v=>'· '+v).join('\n')}\n\n请生成一份完整的 PRD 初稿：\n1. 文档概述（背景、目标、范围）\n2. 用户故事（User Stories）\n3. 功能需求详细说明\n4. 非功能需求\n5. 交互流程说明\n6. 数据需求\n7. 验收标准\n8. 上线计划建议`,
tips:['PRD 初稿需要根据实际情况调整完善','追问"帮我补充竞品在这方面的做法"']},

{id:'req-review',category:'需求管理',title:'需求评审模拟',titleEn:'Review Simulation',desc:'AI 扮演评审专家，提前发现需求漏洞',descEn:'AI acts as a reviewer to find gaps in your requirements',level:'进阶',
steps:[
...
{id:'pricing-strategy',category:'定价策略',title:'定价方案建议',titleEn:'Pricing Strategy',desc:'把产品价值、价值指标、免费层边界和升级触发点拆成可执行的定价推演',descEn:'Turn product value, value metric, free tier boundary, and upgrade triggers into a pricing brief',level:'进阶',
steps:[
...
{id:'ab-pricing',category:'定价策略',title:'AB 定价实验设计',titleEn:'Pricing Experiment',desc:'科学设计定价实验，用数据驱动定价决策',descEn:'Design scientific pricing experiments with data-driven decisions',level:'高级',
steps:[
...
prompt:d=>`你是一个资深的增长实验专家，擅长定价实验设计。\n\n实验目标：${d.goal}\n\n测试变量：\n${d.variables}\n\n可用数据：\n${d.data.map(v=>'· '+v).join('\n')}\n\n请设计完整的 AB 定价实验方案：\n1. 实验假设\n2. 实验设计（分组方式、流量分配、实验周期）\n3. 样本量计算\n4. 核心指标定义\n5. 实验监控方案\n6. 结果判定标准\n7. 风险控制措施`,
tips:['定价实验涉及收入，建议小流量测试后逐步放量','追问"如果实验结果不显著怎么办"']},

{id:'data-anomaly',category:'数据分析',title:'数据异常解读',titleEn:'Anomaly Diagnosis',desc:'描述数据异常，AI 帮你排查原因',descEn:'Describe a data anomaly and get root cause analysis',level:'入门',
steps:[
...
prompt:d=>`你是一个资深数据分析专家，擅长排查数据异常。\n\n异常描述：\n${d.anomaly}\n\n相关信息：\n${d.info}\n\n请帮我排查：\n1. 可能的原因分类（技术问题/外部因素/数据问题/产品问题）\n2. 逐一分析每种可能性的概率\n3. 建议排查的步骤和数据维度\n4. 需要进一步确认的信息\n5. 紧急处理建议`,
tips:['描述越详细，排查方向越准确','追问"帮我设计一个数据异常自动告警方案"']},

{id:'report-gen',category:'数据分析',title:'周报/月报生成',titleEn:'Report Generator',desc:'输入关键信息，AI 生成结构化汇报',descEn:'Generate structured weekly or monthly reports',level:'入门',
steps:[
...
prompt:d=>`你是一个资深产品经理，擅长工作汇报。\n\n请根据以下信息生成一份周报：\n\n核心指标：\n${d.metrics}\n\n关键进展：\n${d.progress}\n\n问题与风险：\n${d.risks}\n\n请输出：\n1. 本周概览（2-3 句话总结）\n2. 核心指标表现（用表格，标注环比变化）\n3. 重点进展\n4. 问题与风险（标注严重程度）\n5. 下周计划建议\n6. 需要协调的资源`,
tips:['周报重在简洁，突出变化和行动','追问"帮我优化这份周报，让它更简洁"']},

{id:'dashboard-design',category:'数据分析',title:'数据看板设计',titleEn:'Dashboard Design',desc:'设计产品数据看板，明确核心监控指标',descEn:'Design product dashboards with key metrics',level:'进阶',
steps:[
...
prompt:d=>`你是一个资深数据分析师，擅长数据看板设计。\n\n产品：${d.product}\n监控目标：\n${d.goals.map(v=>'· '+v).join('\n')}\n\n请设计一个完整的数据看板方案：\n1. 看板分区结构（一级指标 / 二级指标 / 下钻维度）\n2. 每个区域的核心指标定义（指标名、计算方式、目标值）\n3. 建议的图表类型\n4. 告警规则建议\n5. 看板布局建议`,
tips:['先确定北极星指标，再搭建看板','追问"帮我用 SQL 写这些指标的计算逻辑"']},

{id:'monetization',category:'商业化',title:'商业化方案',titleEn:'Monetization Plan',desc:'为你的产品设计完整的商业化方案',descEn:'Design a complete monetization plan for your product',level:'高级',
steps:[
...
prompt:d=>`你是一个资深商业化专家。\n\n产品：${d.product}\n偏好商业模式：${d.model}\n目标市场：${d.market}\n\n请设计完整的商业化方案：\n1. 推荐的商业模式及理由\n2. 收入模型测算（用户数 × 转化率 × ARPU）\n3. 产品分层策略（免费/付费/高级）\n4. 变现路径规划\n5. 关键里程碑和目标\n6. 风险与应对`,
tips:['商业化需要结合实际数据，建议先用小规模验证','追问"帮我做一份 12 个月的商业化路线图"']},

{id:'retention-strategy',category:'商业化',title:'续订挽回策略',titleEn:'Retention Strategy',desc:'设计用户续订和流失挽回策略',descEn:'Design subscription renewal and win-back strategies',level:'高级',
steps:[
...
prompt:d=>`你是一个资深增长产品经理，擅长用户续订和流失挽回。\n\n产品：${d.product}\n\n流失用户特征：\n${d.traits}\n\n请设计完整的续订挽回策略：\n1. 流失预警体系（提前识别高风险用户）\n2. 续订激励方案（折扣/赠品/功能解锁）\n3. 流失挽回触达策略（时机/渠道/话术）\n4. 重新激活方案（回归用户专属优惠）\n5. 长期留存提升策略`,
tips:['挽回成本远低于获客成本，值得投入','追问"帮我设计具体的挽回邮件模板"']},

{id:'growth-experiment',category:'增长策略',title:'增长实验设计',titleEn:'Growth Experiment',desc:'设计科学的增长实验，系统化推动用户增长',descEn:'Design scientific growth experiments',level:'进阶',
steps:[
...
prompt:d=>`你是一个资深增长专家，擅长增长实验设计。\n\n增长目标：${d.goal}\n\n可用杠杆：\n${d.levers.map(v=>'· '+v).join('\n')}\n\n约束条件：\n${d.constraints}\n\n请设计一份增长实验计划：\n1. 增长模型分解（北极星指标 → 关键子指标）\n2. 快赢实验（1-2 周可出结果，低成本）\n3. 核心实验（1-2 月，中投入）\n4. 长期投入（需要持续建设）\n5. 每个实验的假设、设计、预期结果\n6. 实验优先级排序`,
tips:['增长实验要并行跑多个，不要押注单一方案','追问"帮我设计一个推荐裂变方案"']},

{id:'market-entry',category:'增长策略',title:'市场进入策略',titleEn:'Market Entry',desc:'为新市场进入制定策略和行动计划',descEn:'Develop strategies and action plans for new market entry',level:'高级',
steps:[
...
prompt:d=>`你是一个资深战略产品经理，擅长市场进入策略。\n\n产品：${d.product}\n目标市场：${d.market}\n\n可用资源：\n${d.resources.map(v=>'· '+v).join('\n')}\n\n请设计市场进入策略：\n1. 目标市场分析（用户特征、竞争格局、政策环境）\n2. 进入模式建议（直接进入/合作/收购/…）\n3. 产品本地化策略\n4. 获客策略\n5. 里程碑规划（第 1 月/第 3 月/第 6 月/第 12 月）\n6. 风险与应对`,
tips:['市场进入需要充分调研，建议先做小规模验证','追问"帮我分析这个市场的竞争格局"']},

{id:'cross-team-email',category:'项目沟通',title:'跨部门沟通邮件',titleEn:'Cross-team Email',desc:'生成专业的跨部门沟通邮件，推动协作',descEn:'Generate professional cross-team communication emails',level:'入门',
steps:[
...
{id:'project-retrospective',category:'项目沟通',title:'项目复盘报告',titleEn:'Project Retrospective',desc:'生成结构化的项目复盘，沉淀经验教训',descEn:'Generate structured project retrospectives',level:'进阶',
steps:[
